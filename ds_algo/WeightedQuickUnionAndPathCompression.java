...
public class WeightedQuickUnionAndPathCompression {

    int arr[];
    int size[];

    public WeightedQuickUnionAndPathCompression(int n)
    {
        arr = new int[n];
        size = new int[n];
        for(int i=0;i<n;i++)
    ...
    public int root(int p)
    {
        ...
        while(p != arr[p])
        {
            ///this will point to grand parent of the current parent
            arr[p] = arr[arr[p]];
            ///we are making every node point to its grandparent
            //we are chaning array data
            //this will keep the tree almost flat
            p=arr[p];
